...
app.post('/Carrera', async (req, res) => {    
    let conn;
    conn = await pool.getConnection();
    const data = await conn.query("INSERT INTO Carrera values (?,?,?)", [0, req.body.Nombre, req.body.Duracion]);
    console.log(data);
    res.json(data);
});
...
app.post('/Materia', async (req, res) => {    
    let conn;
    conn = await pool.getConnection();
    const data = await conn.query("INSERT INTO Materia values (?,?,?,?)", [0, req.body.Nombre, req.body.Grado, req.body.ID_Carrera]);
    console.log(data);
    res.json(data);
});
...
app.post('/Grupo', async (req, res) => {    
    let conn;
    conn = await pool.getConnection();
    const data = await conn.query("INSERT INTO Grupo values (?,?,?)", [0, req.body.Nombre, req.body.Grado]);
    console.log(data);
    res.json(data);
});
...
app.post('/Horario', async (req, res) => {    
    let conn;
    conn = await pool.getConnection();
    const data = await conn.query("INSERT INTO Grupo values (?,?,?,?,?)", [0, req.body.Hora, req.body.Dia, req.body.ID_Grupo, req.body.ID_Materia]);
    console.log(data);
    res.json(data);
});
